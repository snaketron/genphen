...
# Description:
# Given a matrix of genotypes (SAAPs as columns) and a corresponding vector of
# phenotypes, this function computes the association between each amino acid
# substitution pair found within the SAAPs and the phenotype vector.
runGenphenSaap <- function(genotype = NULL, phenotype = NULL,
                           technique = NULL, fold.cv = 0.66,
                           boots = 100) {
  ...
  checkInputs <- function(genotype, phenotype, technique, fold.cv, boots) {
    ...
    if(is.null(genotype) | is.null(phenotype) | is.null(technique)) {
      stop("arguments genotype, phenotype and technique must be speified.")
    }

    # is it matrix/dataframe or AAMultipleAlignment
    if(is.null(attr(genotype, "class")) == FALSE) {
      if(!attr(genotype, "class") %in% c("AAMultipleAlignment")) {
        stop("genotype can be one of the following structures: matrix or
             data.frame or AAMultipleAlignment")
      }
      else {
  ...
  # the genotype from the phenotype alone (and corresponding CIs).
  # The classification is non-linear and is computed using random forests.
  ...
  computeRfClassification <- function(entry, fold.cv, boots) {
    ...
    if(length(result) != 9) {
      result <- list(ca = NA, ca.ci.low = NA, ca.ci.high = NA,
                     ca.ci.length = NA, ca.boots = NA, kappa.mean = NA,
                     kappa.ci.low = NA, kappa.ci.high = NA,
                     kappa.ci.length = NA)
    }
    return(result)
  }
  ...
  # Description:
  # Given two vectors one dependent (genotype) and one independent (phenotype),
  # compute the classification error of classifying the genotype from the
  # phenotype alone (and corresponding CIs). The classification is linear and
  # is computed using linear support vector machines.
  # Return: 3-element vector (mean.classification, ci.low, ci.high)
  ...
  setResultTypes <- function(results) {
    ...
    # all the col names
    cols <- c("site", "aa1", "aa2", "count.aa1", "count.aa2",
    ...
    # numerical col names
    num.cols <- c("count.aa1", "count.aa2",
                  "effect.size", "effect.CI.low", "effect.CI.high",
                  "ca", "ca.CI.low", "ca.CI.high", "ca.CI.length", "ca.boots",
                  "kappa", "kappa.CI.low", "kappa.CI.high", "kappa.CI.length",
                  "anova.score")
          ...
          for(s2 in (s1 + 1):length(states)) {
            ...
            if(any(table(entry$g) == 1)) {
              ...
              row <- c(site.id, general$aa1, general$aa2, general$aa1.nr,
                       general$aa2.nr, rep(x = NA, times = 13))
              results <- rbind(results, row)
            }
            else {
              # compute scores
              ...
              row <- c(site.id,
                       ...
                       class$kappa.mean, class$kappa.ci.low,
                       class$kappa.ci.high, class$kappa.ci.length,
                       anova.score)
              results <- rbind(results, row)
            }
          }
  ...
  checkInputs(genotype = genotype, phenotype =  phenotype,
              technique = technique, fold.cv = fold.cv, boots = boots)
