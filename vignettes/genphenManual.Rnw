...
sequences (individuals), each composed of 7 amino acids (positions), and a 
...
genetic states found at each of the 7 studied positions in the multiple 
sequence alignment.
...
chains} composed of \underline{2500 iterations} each, including \underline{500 
warmup iterations}. 
\item \underline{Random forest} was selected as for the statistical learning, 
which will be run in a cross-validation mode with \underline{300 iterations}. 
\item All estimates will be reported according to their mean and \underline{95\% 
HDI} 
\item Retrospective power analysis with \underline{10 iterations} will be 
performed. 
...
beyond the rank of 1000. We can thus narrow down our interval of interest to
the top-ranked 2500 SNPs, yielding massive data reduction of 95\%, while still
